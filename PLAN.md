...
Phase 6 (`openshell-environments`' `bin/clean-machine-acceptance`) is complete: 23/23 checks pass against this repository's published `pi-openshell-v0.1.0` release inside a fresh container with no checkout of either repository, including a live Pi conversation through the real `pi-codex` provider and a live `terraform apply` block via `extensions/terraform-guard`. A real machine's `pi` was switched over to it.

Real use immediately surfaced a bug acceptance hadn't covered: npm couldn't write to its cache inside a real sandbox (`openshell-environments`' toolchain-entrypoint env restoration, already handled for Rust, was never extended to npm/uv). Fixed there and released as `openshell-environments` `0.3.0`; this repository re-pinned `release/openshell-environments.version` to `0.3.0` and published `pi-openshell-v0.2.0`, independently re-verified against a live sandbox on the real machine's actual gateway, not just an isolated test container.

The earlier OpenShell migration remains the rollback baseline.
