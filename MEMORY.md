...
- OpenShell 0.0.102 treats an omitted `network_policies` block as no outbound authorization, not unrestricted internet: its proxy returns HTTP 403 for tunnels and Node surfaces this as `fetch failed`. Keep explicit OpenAI endpoint and `node`/`pi` binary rules in `openshell-policy.yaml`.
