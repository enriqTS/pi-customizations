...
- The Debian Node image is intentionally retained as a small general-purpose base and adds Python/uv/ruff, Rust/Cargo/rustfmt, the latest Terraform release, and native-build prerequisites. Tool caches use `/tmp` because the OpenShell filesystem policy does not allow writes to the general home directory. Pin Terraform when a project requires reproducible versioning.
